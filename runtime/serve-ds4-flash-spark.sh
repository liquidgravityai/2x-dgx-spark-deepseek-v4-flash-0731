#!/usr/bin/env bash
# Derived from the Apache-2.0-licensed vLLM Spark launcher.
# Liquid Gravity AI modifications require explicit per-node network settings
# and expose validated DSpark proposal-selection controls.
...
require_env() {
  local name=$1
  if [[ -z "${!name:-}" ]]; then
    printf '%s must be set for this node\n' "${name}" >&2
    exit 2
  fi
}

for name in \
  NODE_RANK MASTER_ADDR VLLM_HOST_IP \
  NCCL_IB_HCA NCCL_IB_GID_INDEX \
  NCCL_SOCKET_IFNAME GLOO_SOCKET_IFNAME UCX_NET_DEVICES TP_SOCKET_IFNAME
do
  require_env "${name}"
done
...
dspark_draft_sample_method=${DSPARK_DRAFT_SAMPLE_METHOD:-probabilistic}
...
if [[ "${enable_dspark}" == 1 ]]; then
  ...
  case "${dspark_draft_sample_method}" in
    ...
    *)
      printf 'DSPARK_DRAFT_SAMPLE_METHOD must be greedy or probabilistic\n' >&2
      exit 2
      ;;
  esac
fi
...
if [[ "${enable_dspark}" == 1 ]] && ((num_speculative_tokens > 0)); then
  ...
  speculative_config=$(printf \
    '{"method":"dspark","num_speculative_tokens":%s,"draft_sample_method":"%s"%s%s}' \
    "${num_speculative_tokens}" "${dspark_draft_sample_method}" \
    "${draft_attention_json}" "${gating_json}")
  speculative_args+=(
    --speculative-config
    "${speculative_config}"
  )
